        ...
        try {
            ...
            return [type, (Array.isArray(result) ? result : [result]).map(r => {
                    if (typeof r === 'object' && r !== null && 'exchange' in r) {
                        return `${r.exchange} (priority ${r.priority})`;
                    }
                    return String(r);
                })];
        }
...
function checkEmailSecurity(records) {
...
function extractSeoMeta(headers, body) {
    ...
    return new Promise((resolve) => {
        const socket = tls.connect(port, domain, { servername: domain, timeout: 10000 }, () => {
            ...
            if (!cert || !cert.valid_to)
                return resolve({ valid: false, expired: true, error: 'No certificate returned' });
...
server.registerTool('tech_detect', {
    ...
    if (error && !body)
        return { content: [{ type: 'text', text: JSON.stringify({ url, error }, null, 2) }] };
    ...
    return {
        content: [{ type: 'text', text: JSON.stringify({
                    ...
                    analysis: {
                        ...
                        social: social,
                        domain: registrar,
                        dns: Object.fromEntries(Object.entries(dnsRecords).filter(([_, v]) => v.length > 0)),
                    },
                }, null, 2) }],
    };
});
