function hasHeaderStartingWith(headers: Record<string, string>, prefix: string): boolean {
  return Object.keys(headers).some(k => k.startsWith(prefix));
}

function headerContains(headers: Record<string, string>, key: string, search: string): boolean {
...
export const patterns: TechPattern[] = [
  // CMS
...
export function detectTech(headers: Record<string, string>, body: string) {
  ...
  for (const p of patterns) {
    try {
      if (p.check(headers, body)) detected.push({ name: p.name, category: p.category });
